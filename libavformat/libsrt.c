...
typedef struct SRTContext {
  ...
  /* Multi-client support for listener mode */
  int is_output;
...
{   
  ...
  if (timeNow > s->lastStatsTime + s->stats) {
    ...
    static void* libsrt_listener_thread(void *arg)
    {
        ...
        if (ret < 0) {
          ...
          static int libsrt_setup(URLContext *h, const char *uri, int flags)
          {
            ...
            if (ret) {
              ...
              goto fail1;
              if (s->mode == SRT_MODE_LISTENER) {
                // For output streams in listener mode, start multi-client support
                if (flags & AVIO_FLAG_WRITE) {
                  ...
                  if (srt_listen(fd, s->max_connections)) {
                    ret = libsrt_neterrno(h);
                    srt_epoll_release(write_eid);
                    goto fail1;
                  }
                  
                  /* Initialize multi-client structures */
                  pthread_mutex_init(&s->clients_mutex, NULL);
                  s->num_clients = 0;
                  memset(s->clients, 0, sizeof(s->clients));
                  
                  /* Create listener epoll for accepting connections */
                  s->listener_eid = srt_epoll_create();
                  if (s->listener_eid < 0) {
                    ret = libsrt_neterrno(h);
                    srt_epoll_release(write_eid);
                    goto fail1;
                  ...
                  if (srt_epoll_add_usock(s->listener_eid, fd, &modes) < 0) {
                    ret = libsrt_neterrno(h);
                    srt_epoll_release(s->listener_eid);
                    srt_epoll_release(write_eid);
                    goto fail1;
                  }
                  
                  /* Start listener thread */
                  s->listener_thread_running = 1;
                  ret = pthread_create(&s->listener_thread, NULL, libsrt_listener_thread, h);
                  if (ret != 0) {
                    av_log(h, AV_LOG_ERROR, "Failed to create listener thread\n");
                    ret = AVERROR(ret);
                    srt_epoll_release(s->listener_eid);
                    srt_epoll_release(write_eid);
                    goto fail1;
                  ...
                  av_log(h, AV_LOG_INFO, "SRT multi-client listener started on port %d\n", port);
                  srt_epoll_release(write_eid);
                } else {
                  // For input streams, use single client mode
              ...
              static int libsrt_open(URLContext *h, const char *uri, int flags)
              {
                ...
                if (p) {
                  ...
                  if (av_find_info_tag(buf, sizeof(buf), "max_connections", p)) {
                    s->max_connections = strtol(buf, NULL, 10);
                    if (s->max_connections < 1 || s->max_connections > MAX_CLIENTS) {
                      av_log(h, AV_LOG_WARNING, "max_connections value %d out of range [1,%d], using default\n", 
                ...
                static int libsrt_write(URLContext *h, const uint8_t *buf, int size)
                {
                  ...
                  if (s->mode == SRT_MODE_LISTENER && s->is_output) {
                    ...
                    // Always return success even if no clients are connected
                    return size;
                  }
                ...
                static int libsrt_close(URLContext *h)
                {
                  ...
                  if (s->mode == SRT_MODE_LISTENER && s->is_output && s->listener_thread_running) {
                    s->listener_thread_running = 0;
                    pthread_join(s->listener_thread, NULL);
